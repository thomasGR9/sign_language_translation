...
def optimized_semantic_smoothing_loss(logits, L_index, L_values, label_mask=None, single_word_multiplier=1):
    """
    Highly optimized semantic smoothing loss using a single scatter operation.
    Applies 2x penalty to single-word translations to encourage cross-attention learning.
    """
    ...
    # Apply mask if provided
    if label_mask is not None:
        # Apply mask to token losses
        masked_token_losses = token_losses * label_mask.float()  # (batch_size, seq_len)
        
        # Calculate valid tokens per sequence
        valid_tokens_per_seq = label_mask.sum(dim=1)  # (batch_size,)
        
        # Calculate per-sequence losses
        per_seq_losses = masked_token_losses.sum(dim=1) / valid_tokens_per_seq.clamp(min=1)  # (batch_size,)
        
        # Apply x weight to single-word translations (sequences with exactly 1 valid token)
        # This encourages the model to rely on cross-attention rather than self-attention
        single_word_mask_float = (valid_tokens_per_seq == 2).float()  # (batch_size,)
        penalty_multiplier = single_word_mask_float * (single_word_multiplier - 1.0) + 1.0
        per_seq_losses = per_seq_losses * penalty_multiplier

        # Average across all sequences
        total_loss = per_seq_losses.mean()
    else:
        # If no mask, use all tokens
    ...
    def forward(self, hidden_states, video_representations, video_mask=None, stride=1):
        ...
        try:
            cross_attention_output, _ = self.multihead_attn(
                query=hidden_states,                  # From GPT tokens
            ...
            hidden_states = cross_attention_output+residual
...
class VideoGPT(nn.Module):
    ...
    """
    def __init__(self, model_name="distilgpt2", num_cross_heads=12, freeze_gpt=True, trainable_lm_head=True, trainable_final_ln=True, adapter_dim=384, stride=2, unfreeze_last_n_blocks=0):
    ...
    def _freeze_gpt_parameters(self):
        ...
        total_blocks = len(self.gpt.transformer.h)
        if self.unfreeze_last_n_blocks > 0:
            blocks_to_unfreeze = min(self.unfreeze_last_n_blocks, total_blocks)
            starting_idx = total_blocks - blocks_to_unfreeze
            
            for i in range(starting_idx, total_blocks):
                print(f"Unfreezing GPT block {i}")
                for param in self.gpt.transformer.h[i].parameters():
                    param.requires_grad = True


    def set_unfrozen_blocks(self, n_blocks):
        """Explicitly set how many blocks to unfreeze"""
        prev_value = self.unfreeze_last_n_blocks
        self.unfreeze_last_n_blocks = n_blocks
        self._freeze_gpt_parameters()
        print(f"Changed unfrozen blocks from {prev_value} to {n_blocks}")
        
    # Standard state dict methods
    def state_dict(self):
        state = super().state_dict()
        state['_unfreeze_config'] = self.unfreeze_last_n_blocks
        return state
        
    def load_state_dict(self, state_dict, strict=True):
        # Extract unfreezing config if present
        if '_unfreeze_config' in state_dict:
            self.unfreeze_last_n_blocks = state_dict.pop('_unfreeze_config')
            self._freeze_gpt_parameters()
...
class VideoGPT_new(nn.Module):
    """
    Integrates pre-trained GPT-2 with cross-attention for video-to-text translation.
    """
    def __init__(self, model_name="distilgpt2", num_cross_heads=12, number_of_blocks=5, freeze_gpt=True, stride=2):
        super(VideoGPT_new, self).__init__()
        
        # Load pre-trained model
        gpt = AutoModelForCausalLM.from_pretrained(model_name)
        gpt.to("cuda")
        self.config = gpt.config
        self.embeddings = gpt.transformer.wte
        self.tokens_potisional_encodings = gpt.transformer.wpe
        self.vocab_size = gpt.config.vocab_size
        ...
        self.post_self_att_norms = nn.ModuleList([
            nn.LayerNorm(self.hidden_size, eps=1e-5)
            for _ in range(self.number_of_blocks)
        ])
        ...
        self.transformer_FFNs = nn.ModuleList([
            transformer_FFN(self.hidden_size) 
            for _ in range(self.number_of_blocks)
        ])

        self.lm_head = nn.Linear(self.hidden_size, self.vocab_size)
        self.final_ln = nn.LayerNorm(self.hidden_size, eps=1e-5)


        
        if freeze_gpt:
            self._freeze_gpt_embeddings()
    
    def _freeze_gpt_embeddings(self):
        """Freeze embeddings of GPT model."""
        for param in self.embeddings.parameters():
            param.requires_grad = False
            print(f"Freezed embeddings")
        for param in self.tokens_potisional_encodings.parameters():
            param.requires_grad = False
            print(f"Freezed positional encodings")
    ...
    def forward(self, input_ids, video_representations, video_mask=None, 
        ...
        # Get GPT embeddings (word + position)
        position_ids = torch.arange(0, n_tokens, device=input_ids.device).unsqueeze(0).expand(batch_size, -1)
        gpt_embeds = self.embeddings(input_ids) + self.tokens_potisional_encodings(position_ids)
        ...
        if label_mask is not None:
            # 1. Key padding mask - prevents attention to padding tokens
            # True = ignore this position (it's padding)
            key_padding_mask = ~label_mask  # Shape: [batch_size, seq_length]

            # 2. Causal attention mask - enforces autoregressive property
            seq_length = label_mask.size(1)

            # Create a mask where True is in upper triangle (positions NOT allowed to attend)
            # This creates a mask where position i can only attend to positions j where j ≤ i
            causal_mask = ~torch.tril(torch.ones((seq_length, seq_length), dtype=torch.bool, 
                                               device=label_mask.device))
            attn_mask = causal_mask  # Shape: [seq_length, seq_length]


        for i in range(self.number_of_blocks):
            #hidden_states = torch.clamp(hidden_states, min=-50, max=50)
            self_attn_output, _ = self.self_attentions[i](
            query=hidden_states,
            key=hidden_states,
            value=hidden_states,
            attn_mask=attn_mask,
            ...
            hidden_states = self.post_self_att_norms[i](hidden_states)
            ...
            hidden_states = self.transformer_FFNs[i](cross_attention_output)
...
def train_asl_model(
    ...
    if optimizer==None:
        print(f"Initializing new Adam optimizer with weight_decay: {weight_decay}, and learning rate: {learning_rate}")
    ...
    def forward_pass(batch):
        ...
        video_representation = models_dict['temporal_transformer'](
    ...
    try:
        for epoch in range(num_epochs):
            ...
            # Validation loop
            with torch.no_grad():
                print("Beggining validation phase")
                for batch_idx, batch in enumerate(val_loader):
                    ...
                    loss = outputs['loss']
                    if torch.isnan(loss) or torch.isinf(loss):
                        print(f"Invalid loss value: {loss.item()} in batch {batch_idx}, skipping batch")
                        continue
                    ...
                    if batch_idx < num_examples_to_display:
                        ...
                        for ex in example_comparisons[:min(len(example_comparisons), 5)]:  
                    ...
                    if loss is not None:
                        val_loss += loss.item()
                        if verbose > 1 and batch_idx % log_freq_batches == 0:
                            print(f"Batch {batch_idx+1}/{val_batches} - Loss: {loss.item():.4f}")
            ...
            if verbose > 0:
                ...
                if validation_examples:
                    ...
                    table_data = []
                    for i, ex in enumerate(validation_examples[-10:]):  # Show last 10 examples
...
def resume_training_from_checkpoint(
    ...
    grad_clip_value=3.0,
    weight_decay=1e-3,  #Specify it 
    num_epochs=30,       # Additional epochs to train
    scheduler_type='warmup_plateau',  
    scheduler_params={'warmup_epochs': 3,'warmup_start_factor': 0.1,'mode': 'min', 'factor': 0.1, 'min_lr':1e-6 ,'patience': 20, 'verbose': False},
    ...
    if load_scheduler_and_optimizer:
        ...
        for model in model_components:
            all_params.extend(model.parameters())
        temp_optimizer = optim.Adam(all_params, weight_decay=weight_decay)
        temp_scheduler_params = scheduler_params or {'warmup_epochs': 3,'warmup_start_factor': 0.01,'mode': 'min', 'factor': 0.5, 'patience': 5, 'verbose': False}
        temp_scheduler = WarmupToPlateauScheduler(temp_optimizer, **temp_scheduler_params)
        # Load checkpoint
        epoch, history, extra_info = load_checkpoint(
            checkpoint_path, models_dict, device, temp_optimizer, temp_scheduler
        )
    else:
        epoch, history, extra_info = load_checkpoint(
            checkpoint_path, models_dict, device, None, None
        )
    ...
    if load_scheduler_and_optimizer:
        return train_asl_model(
            ...
            # Validation and visualization
            num_examples_to_display=num_examples_to_display,  # Number of examples to display during validation
            # Logging
        ...
        return train_asl_model(
            ...
            scheduler=None,  
            grad_clip_value=grad_clip_value,
            weight_decay=weight_decay,  #Specify it 
            num_epochs=num_epochs,
            scheduler_type=scheduler_type,  
            scheduler_params=scheduler_params,
            early_stopping=early_stopping,
...
def load_checkpoint(path, models_dict, device, optimizer=None, scheduler=None):
    ...
    for name, model in models_dict.items():
        ...
        if model_key in checkpoint:
            try:
                ...
                print(f"Succesfully loaded model: {model_key}")
            except Exception as e:
                print(f"Failed to save load model: {model_key}, because of: {e}")

    
    # Load optimizer if provided
    if optimizer is not None and 'optimizer' in checkpoint:
        optimizer.load_state_dict(checkpoint['optimizer'])
        print(f"Succesfully loaded optimizer, beggining with learning rate {optimizer.param_groups[0]['lr']}")
    
    # Load scheduler if provided
    if scheduler is not None and 'scheduler' in checkpoint:
        scheduler.load_state_dict(checkpoint['scheduler'])
        print(f"Succesfully loaded scheduler")
